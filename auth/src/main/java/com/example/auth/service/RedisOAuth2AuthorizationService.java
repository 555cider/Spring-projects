...
import com.example.auth.util.SerializationUtil;
    ...
    public void save(OAuth2Authorization authorization) {
        ...
        if (oAuth2AuthorizationGrantAuthorization instanceof OAuth2AuthorizationCodeGrantAuthorization codeAuthz) {
            template.opsForValue()
                    .set(OAUTH2_GRANT_PREFIX + codeAuthz.getAuthorizationCode().getTokenValue(), SerializationUtil.serialize(codeAuthz), CODE_EXPIRATION)
                    .subscribe();
            return;
        }
...
//        }
    ...
    public OAuth2Authorization findById(String id) {
        ...
        return template
                ...
                .map(a -> SerializationUtil.deserialize(a, OAuth2Authorization.class))
                .block();
    }
    ...
    public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
        ...
        if (new OAuth2TokenType(OAuth2ParameterNames.CODE).equals(tokenType)) {
            return template.opsForValue()
                    ...
                    .map(str -> SerializationUtil.deserialize(str, OAuth2AuthorizationCodeGrantAuthorization.class))
                    .mapNotNull(this::toOAuth2Authorization)
                    .block();
        }
        return template.opsForValue()
                .get(OAUTH2_GRANT_PREFIX + token)
                .map(str -> SerializationUtil.deserialize(str, OAuth2ClientCredentialsGrantAuthorization.class))
                .mapNotNull(this::toOAuth2Authorization)
                .block();
    }
